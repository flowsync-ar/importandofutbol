...
export function Catalog({ products, initialCategory }: { products: Product[]; initialCategory?: string }) {
  ...
  return <div className="catalog-layout">
    {filtersOpen && <button className="filter-backdrop" aria-label="Cerrar filtros" onClick={() => setFiltersOpen(false)}/>}<aside className={`filters ${filtersOpen ? "open" : ""}`}><div className="filter-title"><SlidersHorizontal/> Filtros <button onClick={() => setFiltersOpen(false)} aria-label="Cerrar panel de filtros"><X/></button></div><label>Buscar<input aria-label="Buscar camisetas" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Equipo o camiseta"/></label><label>Categoría<select value={category} onChange={(e) => setCategory(e.target.value)}><option>Todas</option><option>Selecciones</option><option>Clubes</option><option>Retro</option></select></label><label>Talle<select value={size} onChange={(e) => setSize(e.target.value)}><option>Todos</option>{["S", "M", "L", "XL"].map((s) => <option key={s}>{s}</option>)}</select></label><button className="button gold apply-filters" onClick={() => setFiltersOpen(false)}>Ver {results.length} resultados</button></aside>

...
export function Catalog({ products, initialCategory, categoryNames }: { products: Product[]; initialCategory?: string; categoryNames?: string[] }) {
  ...
  const results = useMemo(() => products.filter((p) => (!query || `${p.name} ${p.team} ${p.code ?? ""}`.toLowerCase().includes(query.toLowerCase())) && (category === "Todas" || p.category === category) && (size === "Todos" || p.sizes.includes(size))).sort((a, b) => sort === "nombre" ? a.name.localeCompare(b.name) : 0), [products, query, category, size, sort]);

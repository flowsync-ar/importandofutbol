...
export function SiteHeader({ categories }: { categories: Category[] }) {
  ...
  return <>
    <div className="topbar">⚽ Camisetas de fútbol <span>•</span> 📍 De la Pampa al mundo <span>•</span> Consultas por WhatsApp e Instagram</div>
    ...
    {open && <div className="menu-backdrop" onClick={() => setOpen(false)}><aside className="mobile-menu" onClick={(e) => e.stopPropagation()}><button onClick={() => setOpen(false)} aria-label="Cerrar menú"><X/></button><div className="brand"><Image src="/logo.png" width={58} height={58} alt=""/><strong>IMPORTANDO FÚTBOL LP</strong></div><nav><Link href="/" onClick={() => setOpen(false)}>Inicio</Link><Link href="/camisetas" onClick={() => setOpen(false)}>Camisetas</Link><strong className="mobile-cats">Categorías</strong>{categoryLinks.map((link) => <Link key={link.href} href={link.href} onClick={() => setOpen(false)}>{link.label}</Link>)}<Link href="/contacto" onClick={() => setOpen(false)}>Contacto</Link></nav></aside></div>}

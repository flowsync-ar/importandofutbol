...
export default async function AdminPage() {
  ...
  const productsResult = featuredQuery.error
    ...
    : featuredQuery;
  const [categoriesResult, customersResult, sizeGuideResult, contactsResult, adsResult] = await Promise.all([
    ...
    supabase.from("ads").select("id,slot,title,description,href,image_url,active"),
  ]);
  return <AdminProductManager initialProducts={(productsResult.data ?? []) as AdminProduct[]} initialCategories={(categoriesResult.data ?? []) as Category[]} initialCustomers={(customersResult.data ?? []) as Customer[]} initialSizeGuide={(sizeGuideResult.data ?? []) as SizeGuideRow[]} initialContacts={(contactsResult.data ?? []) as ContactChannel[]} initialAds={(adsResult.data ?? []) as Ad[]} email={String(claimsData.claims.email ?? "administrador")} />;
}

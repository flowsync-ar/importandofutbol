...
export default async function HomePage() {
  ...
  const homeAd = adBySlot(ads, "home");
  return <>
    <section className="hero"><div className="container hero-grid"><div className="hero-copy"><span className="eyebrow">TU PASIÓN. TU CAMISETA.</span><h1>FÚTBOL<br/>QUE SE <em>VISTE.</em></h1><p>Descubrí las mas lindas camisetas y consultá tu pedido sin vueltas.</p><div className="hero-actions"><Link className="button gold" href="/camisetas">Ver camisetas <ArrowRight/></Link><Link className="button outline" href={retro ? `/${retro.slug}` : "/camisetas"}>Explorar retro</Link></div></div><div className="hero-art-slot"><HeroFeatured products={products}/></div></div></section>
    {isVisibleAd(homeAd) ? <div className="container home-ad"><AdSpot ad={homeAd}/></div> : null}
    <section className="section container"><header className="section-heading"><div><span className="eyebrow dark">DESTACADAS</span><h2>Las más buscadas</h2></div><Link className="text-link" href="/camisetas">Ver todas <ArrowRight/></Link></header><div className="product-grid">{homeShowcase(products).map((product) => <ProductCard key={product.id} product={product}/>)}</div></section>
